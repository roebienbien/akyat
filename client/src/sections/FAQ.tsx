...
const FAQQuestions = [
  ...
  },
  {
    ...
    answer: 'Pets are allowed on some trails, but restrictions apply. Please check the specific trail details or contact us to confirm if pets are permitted on your chosen route.',
  },
];
...
export default function FAQSection() {
  ...
  return (
    <div id='faq' className='flex h-screen flex-col items-center justify-center gap-y-8 '>
      <h2 className='text-4xl font-semibold'>Frequently Asked Questions</h2>
      <div className='flex flex-col'>
        {FAQQuestions.map((faq, index) => (
          <FAQAccordion key={index} isOpen={openIndex === index} onToggle={() => handleToggle(index)} question={faq.question} answer={faq.answer} />
        ))}
      </div>
    </div>
  );
